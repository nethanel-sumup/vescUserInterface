...
#include "vescLib/buffer.h"
#include "vescLib/confgenerator.h"
#include <string.h>
...
static void (*rx_detect_func)(float cycle_int_limit, float coupling_k, const signed char *hall_table,
    signed char hall_res) = 0;
...
static void (*motor_control_set_func)(mc_control_mode mode, float value) = 0;
...
void bldc_interface_process_packet(unsigned char *data, unsigned int len)
{
  ...
  switch (id)
  {
    ...
    case COMM_GET_MCCONF_DEFAULT:
      confgenerator_deserialize_mcconf(data, &mcconf);
    ...
    case COMM_DETECT_MOTOR_PARAM:
      ...
      if (rx_detect_func)
      {
        rx_detect_func(detect_cycle_int_limit, detect_coupling_k, detect_hall_table, detect_hall_res);
      }
      break;
...
void bldc_interface_set_rx_detect_func(
    void (*func)(float cycle_int_limit, float coupling_k, const signed char *hall_table, signed char hall_res))
{
  rx_detect_func = func;
}
...
void bldc_interface_set_sim_control_function(void (*func)(mc_control_mode mode, float value))
{
...
void bldc_interface_set_duty_cycle(float dutyCycle)
{
  ...
  {
    motor_control_set_func(CONTROL_MODE_DUTY, dutyCycle);
    return;
  }
...
void bldc_interface_set_current(float current)
{
  if (motor_control_set_func)
  {
    motor_control_set_func(CONTROL_MODE_CURRENT, current);
    return;
  }
  int32_t send_index = 0;
...
void bldc_interface_set_current_brake(float current)
{
  if (motor_control_set_func)
  {
    motor_control_set_func(CONTROL_MODE_CURRENT_BRAKE, current);
    return;
  }
  int32_t send_index = 0;
...
void bldc_interface_set_rpm(int rpm)
{
  if (motor_control_set_func)
  {
    motor_control_set_func(CONTROL_MODE_SPEED, rpm);
    return;
  }
  int32_t send_index = 0;
...
void bldc_interface_set_pos(float pos)
{
  if (motor_control_set_func)
  {
    motor_control_set_func(CONTROL_MODE_POS, pos);
    return;
  }
  int32_t send_index = 0;
...
void bldc_interface_set_mcconf(const mc_configuration *mcconf)
{
  ...
  send_index += confgenerator_serialize_mcconf(send_buffer, mcconf);
  send_packet_no_fwd(send_buffer, send_index);
}
...
void bldc_interface_set_appconf(const app_configuration *appconf)
{
  ...
  send_index += confgenerator_serialize_appconf(send_buffer, appconf);
  send_packet_no_fwd(send_buffer, send_index);
}
